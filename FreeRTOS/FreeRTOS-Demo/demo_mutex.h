...
SemaphoreHandle_t g_hMutex;

mTaskDefine(user1_task)
{
    for (;;)
    {
        xSemaphoreTake(g_hMutex, OS_WAIT_FOREVER);

        s_Resource ++;
        xprintf("[%s] take the resource, current value [%08x].\n", __func__, s_Resource);

        xSemaphoreGive(g_hMutex);
    }
}
...
{
    xprintf("\n");
    xprintf("[INFO]: This is a 'mutex' demo.\n");
    xprintf("\n");

    g_hMutex = xSemaphoreCreateMutex();
    if ( !g_hMutex )
    {
