...
#error "Please define a way to initialize a memory block."
	...
	uint32_t total = default_num_contexts*results[0].iterations*1000000/time_in_secs(total_time)*1000/CPU_CLOCK_MHZ;

	if (total_errors > 0)
	{
		xprintf("Errors detected\n");
	} else {
		xprintf("Coremark ticks: %lu\n",(ee_u32)total_time);
		xprintf("Coremark score (Coremarks/MHz): %d.%-d/MHz\n", total/1000, total%1000);
	}
