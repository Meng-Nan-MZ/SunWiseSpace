...
extern volatile unsigned int  ept_count_ms;
/*
...
whetstone_main(void)
{
	...
	long LOOP;
	int II, JJ;
	long start_time_val=0,end_time_vale=0,total_tick;
	/* added for this version */
	long loopstart;
	long startsec, finisec;
	float KIPS;
	int continuous;
	double cost_ns,cost_ms;
...
*/
	//startsec = time(0);
 // PSPRINTF("whetstone test begin loop is %d and cyle %d\n",loopstart,II);

  //dPSPRINTF("whetstone test begin loop is %d and cyle %d\n",loopstart,II);

  //pit count 1ms

//EPT_start(0x6993);//enable timer 1ms
...
*/
   ...
   #if 0  
   cost_ns = ept_count_ms/1000;
   KIPS = (100.0*LOOP*II)/cost_ns;
   PSPRINTF("cost %d ms\n",ept_count_ms);
   F_PSPRINTF(">>>MWIPS is %5.3f>>>>>>\n",(float)((KIPS/1000)/20));  
   #endif
...
POUT(long N, long J, long K, double X1, double X2, double X3, double X4)
{
	...
	PSPRINTF("%7ld %7ld %7ld %12.4e %12.4e %12.4e %12.4e\n",
						N, J, K, X1, X2, X3, X4);
	PSPRINTF("%7ld %7ld %7ld %12.4e %12.4e %12.4e %12.4e\n",
						N, J, K, X1, X2, X3, X4);	
